...
__global__ void precalc_interp_kick(const double * voltage_array,
...
{
    const double inv_bin_width = (bins - 1) / (bin_centers[bins - 1] - bin_centers[0]);
                                   ...
                                   const double acc_kick)
{
    ...
    const double inv_bin_width = (bins - 1) / (bin_centers[bins - 1] - center0);
...
int main(int argc, char const *argv[])
{
    ...
    thrust::device_vector<double> d_volt_kick(n_slices - 1);
    thrust::device_vector<double> d_factor(n_slices - 1);
    ...
    double *d_volt_kick_ptr = thrust::raw_pointer_cast(d_volt_kick.data());
    double *d_factor_ptr = thrust::raw_pointer_cast(d_factor.data());
    ...
    printf("interp_kick_gpu_v9\ttime(ms)\t%d\t0\t1\n", duration);
