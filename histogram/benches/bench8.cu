...
// Shared memory atomics histogram
...
//const double inv_bin_width)
{
    ...
    for (size_t i = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        if ((input[i] < cut_left) || (input[i] > cut_right))
            continue;
        const int ffbin = (int) ((input[i] - cut_left) * inv_bin_width);
        atomicAdd( &sh_hist[ffbin], 1);
    }
    __syncthreads();
...
int main(int argc, char const *argv[])
{
    ...
    for (int i = 0; i < n_turns; ++i) {
        ...
        histogram <<< blocks, threads, n_slices * sizeof(int)>>>(
    ...
    printf("histogram_gpu_v1\ttime(ms)\t%d\t0\t1\n", duration);
