...
result_dir = home + '{}/results/csv/roofline1/'
image_name = home + '/results/plots/roofline1/roofline.pdf'
...
def plot_roofline(roofs, flops, x_key, y_key):
    ...
    for name, value, roof_type in zip(roofs[0], roofs[1], roofs[2]):
    ...
    for tc, flop in flops.items():
        plt.plot(flop[x_key], flop[y_key], 'o',
                 label='%s (%.2f, %.1f) ' % (tc, flop[x_key], flop[y_key]),
                 markersize=10)
...
if __name__ == '__main__':
    ...
    for tc in testcases:
        for dirpath, _, files in os.walk(result_dir.format(tc)):
            for f in files:
                if 'roofs.csv' in f:
                    roofs_file = os.path.join(dirpath, f)
                elif 'flops.csv' in f:
                    flops_file = os.path.join(dirpath, f)

        data = get_roofs(tc, roofs_file)
        if not roofs:
            roofs = [data[:, 0], np.array(data[:, 1], float), data[:, 2]]
        else:
            roofs[1] += np.array(data[:, 1], float)
